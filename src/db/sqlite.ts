...
export default {
  insertString: ({ id, value, properties, created_at }: any) =>
    insertString.run(id, value, properties, created_at),
  getStringById: (id: string) => getById.get(id),
  getStringByValue: (value: string) => getByValue.get(value),
  getAllStrings: () => getAll.all(),
  deleteStringById: (id: string) => deleteById.run(id)
};
